...
def compute(reg_to_check, comp_op, val_to_compare_with):
    left = str(registers[reg_to_check])
    # register value [comparison operator] compared_value
    return eval(left + comp_op + val_to_compare_with)
...
for code_line in code_lines:
    ...
    if compute(reg_to_check, comp_op, val_to_compare_with):
        if op == 'inc':
